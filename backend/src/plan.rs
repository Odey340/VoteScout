...
fn build_prompt(req: &VotingPlanRequest) -> String {
    ...
    format!(
        "You are VoteScout, a nonpartisan voting assistant. Write a personalized plain-text \
...
pub fn build_ics(req: &VotingPlanRequest) -> String {
    let mut lines: Vec<String> = vec![
        ...
        "VERSION:2.0".into(),
        "PRODID:-//VoteScout//Voting Plan//EN".into(),
    ...
    for (i, e) in req.elections.iter().enumerate() {
        ...
        if !e.early_vote_sites.is_empty() {
            ...
            description.push_str("Early voting available — see your VoteScout plan.");
        }
        if description.is_empty() {
            description.push_str("Check your local election office for polling details.");
        }
        description.push_str("\nGenerated by VoteScout. Verify with official sources.");
        ...
        lines.push(format!("UID:votescout-{start}-{i}@votescout.local"));
